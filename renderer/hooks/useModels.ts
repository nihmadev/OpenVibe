...
export function useModels(config: VibeConfig | null, setConfig: React.Dispatch<React.SetStateAction<VibeConfig | null>>, settingsOpen: boolean) {
  const [enabledModels, setEnabledModels] = useState<Set<string>>(new Set());

  useEffect(() => {
    window.vibe.models.listEnabled().then((ids) => {
