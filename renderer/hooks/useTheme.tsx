...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    Promise.all([
      window.vibe.state.get(THEME_KEY),
      window.vibe.state.get(SCHEME_KEY),
    ]).then(([themeId, scheme]) => {
  ...
  return (
    <ThemeContext.Provider value={{ currentTheme, previewTheme, colorScheme, setTheme, setColorScheme, preview }}>
      {children}
