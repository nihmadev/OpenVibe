...
export function Settings({ open, onClose, onProviderChanged, initialTab, onLanguageChange }: Props): React.ReactElement | null {
  ...
  const [hotkeys, setHotkeys] = useState([
    { id: "open-settings", name: "Open Settings", key: "Ctrl+," },
    { id: "new-chat", name: "New Chat", key: "Ctrl+N" },
    { id: "clear-chat", name: "Clear Chat", key: "Ctrl+L" },
    { id: "focus-input", name: "Focus Input", key: "/" },
  ]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      window.vibe.models.listEnabled(),
  ...
  async function fetchModels(providerList: Provider[]): Promise<void> {
    ...
    await Promise.all(connected.map(async (p) => {
      ...
      if (!res.ok) { console.error("Failed to fetch models for", providerName, res.error); return; }
      ...
      <div className="settings__container" onClick={(e) => e.stopPropagation()}>
        <div className="settings__sidebar">
          <div className="settings__sidebar-group">
            ...
            {renderSidebarItem("general", t("general"), (
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V22a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>
            ))}
            {renderSidebarItem("hotkeys", t("hotkeys"), (
          ...
          <div className="settings__sidebar-group">
            ...
            {renderSidebarItem("providers", t("providers"), (
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/><line x1="6" y1="6" x2="6" y2="6"/><line x1="6" y1="18" x2="6" y2="18"/></svg>
            ))}
            {renderSidebarItem("models", t("models"), (
        ...
        <div className="settings__content">
          <div className="settings__content-header">
            <h2>{activeTab === "general" ? t("general") : activeTab === "models" ? t("models") : activeTab === "providers" ? t("providers") : t("hotkeys")}</h2>
            <button className="settings__close" onClick={onClose}>×</button>
          </div>

          <div className={`settings__content-body${searchStuck ? " settings__content-body--search-stuck" : ""}`} ref={bodyRef}>
            {activeTab === "general" ? (
              <>
                <div className="settings__subsection">
                  <div className="settings__control-group">
                    ...
                    <Select value={general.language} options={languageOptions.map((o) => ({ value: o.value, label: t("lang" + o.value) }))} onChange={(v) => updateGeneral("language", v)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                      ...
                      <input type="checkbox" className="settings__checkbox" checked={general.autoAccept} onChange={(e) => updateGeneral("autoAccept", e.target.checked)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                    ...
                    <Select value={general.terminalShell} options={[{ value: "powershell", label: "PowerShell" }, { value: "cmd", label: "CMD" }, { value: "bash", label: "Bash" }]} onChange={(v) => updateGeneral("terminalShell", v)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                      ...
                      <input type="checkbox" className="settings__checkbox" checked={general.showThinking} onChange={(e) => updateGeneral("showThinking", e.target.checked)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="settings__subsection">
                  ...
                  <div className="settings__control-group">
                    ...
                    <Select value={currentTheme.id} options={themes.map((t) => ({ value: t.id, label: t.name }))} onChange={(v) => setTheme(v)} onHover={(v) => preview(v)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                    ...
                    <Select value={colorScheme} options={[{ value: "dark", label: t("dark") }, { value: "light", label: t("light") }, { value: "system", label: t("system") }]} onChange={(v) => setColorScheme(v as "dark" | "light" | "system")} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                    ...
                    <Select value={general.font} options={[{ value: "Segoe UI", label: "Segoe UI" }, { value: "System", label: t("systemFont") }, ...FONT_OPTIONS]} onChange={(v) => updateGeneral("font", v)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                    ...
                    <Select value={general.codeFont} options={[{ value: "Cascadia Code", label: "Cascadia Code" }, { value: "Consolas", label: "Consolas" }, { value: "monospace", label: "Monospace" }, ...CODE_FONT_OPTIONS]} onChange={(v) => updateGeneral("codeFont", v)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="settings__subsection">
                  ...
                  <div className="settings__control-group">
                    <div className="settings__control-row">
                      ...
                      <input type="checkbox" className="settings__checkbox" checked={general.soundEnabled} onChange={(e) => updateGeneral("soundEnabled", e.target.checked)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                      ...
                      <input type="checkbox" className="settings__checkbox" checked={general.soundOnComplete} onChange={(e) => updateGeneral("soundOnComplete", e.target.checked)} />
                    </div>
                    <div className="settings__control-row">
                      <div className="settings__control-info">
                      ...
                      <input type="checkbox" className="settings__checkbox" checked={general.soundOnStop} onChange={(e) => updateGeneral("soundOnStop", e.target.checked)} />
                    </div>
                  </div>
                </div>
              </>
                        ...
                        return (
                          <div key={p.id} className="settings__provider-row">
                            <div className="settings__provider-info">
                              {template && <img src={`icons/providers/${template.icon}`} className="settings__provider-icon" alt="" />}
                              <div className="settings__provider-name">
                                {p.name}
                              </div>
                            </div>
                            <div className="settings__provider-actions">
                              <button className="settings__edit-btn" onClick={() => startEdit(p)} title={t("editProvider")}>
                  ...
                  <div className="settings__providers-list">
                    {PROVIDER_TEMPLATES.map((tmpl) => (
                      <div key={tmpl.id} className="settings__provider-row">
                        <div className="settings__provider-info">
                          <img src={`icons/providers/${tmpl.icon}`} className="settings__provider-icon" alt="" />
              ...
              <div className="settings__models">
                ...
                {modelsLoading ? (
                  ...
                  const filtered = query
                    ? discoveredModels.filter((m) => m.name.toLowerCase().includes(query) || m.id.toLowerCase().includes(query))
                    : discoveredModels;
                      ...
                      {groupsArray.map(([providerId, models]) => {
                        ...
                        return (
                          <div key={providerId} className="settings__models-group">
                            <div className="settings__models-group-header" onClick={() => toggleProviderCollapse(providerId)}>
                              <ChevronRightIcon open={!isCollapsed} />
                              {icon && <img src={`icons/providers/${icon}`} className="settings__provider-icon" alt="" />}
                              <span>{name}</span>
                            </div>
                ...
                <div className="settings__hotkeys-list">
                  {hotkeys.map((h) => (
                    <div key={h.id} className="settings__control-row">
                      <div className="settings__control-info">
                        <div className="settings__control-label">{h.name}</div>
                      </div>
                      <button className="settings__hotkey-btn">
                        {h.key}
                      </button>
                    </div>
                  ))}
          ...
          onConnect={async (formData) => {
            if (editing.editId) {
              const updated = providers.map((p) =>
                p.id === editing.editId
                  ? { ...p, name: formData.name || p.name, apiKey: formData.apiKey, model: formData.model, baseUrl: formData.baseUrl }
                  : p,
              );
