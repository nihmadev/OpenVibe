...
export function EditorArea({
  ...
  const breadcrumb = React.useMemo(() => {
    ...
    const rel = fileNorm.startsWith(cwdNorm + "/")
      ? fileNorm.slice(cwdNorm.length + 1)
      : fileNorm;
          ...
          {breadcrumb.map((seg, i) => {
            ...
            return (
              <React.Fragment key={i}>
                ...
                <span
                  className={
                    "editor-area__breadcrumb-seg" +
                    (isLast ? " editor-area__breadcrumb-seg--file" : "")
                  }
                >
                  <span className="editor-area__breadcrumb-icon">
                    {isLast
                      ? <FileIcon name={seg} />
                      : <FolderIcon open={false} name={seg} />
                    }
                  </span>
                  {seg}
                </span>
