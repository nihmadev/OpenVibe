...
import { canNavigateHistoryAtCursor, navigatePromptHistory, prependHistoryEntry, HistoryEntry } from "./history.js"
import { normalizePaste, pasteMode } from "./paste.js"
import { promptPlaceholder } from "./placeholder.js"
import { PromptDragOverlay } from "./drag-overlay.js"
...
const ACCEPTED_FILE_TYPES = "image/*,.pdf,.txt,.md,.json,.js,.ts,.jsx,.tsx,.css,.html,.py,.java,.go,.rs,.rb,.c,.cpp,.h,.hpp,.yaml,.yml,.toml,.xml,.csv"
...
export function PromptInput({ disabled, workspace, onSubmit, onStop, models, currentModel, onPickModel, onOpenSettings }: Props): React.ReactElement {
  const { t } = useI18n()
  ...
  const [mentionState, setMentionState] = useState<MentionState>({
    active: false, start: 0, query: "", selected: 0, matches: [], loading: false,
  })
  ...
  const placeholder = useMemo(() =>
    promptPlaceholder({ mode, disabled, suggest: true, example: mode === "shell" ? "git status" : "help me refactor", t }),
  [mode, disabled, t])

  // ─── DOM helpers ────────────────────────────────────────

  const editorText = useCallback((): string =>
    (editorRef.current?.textContent ?? "").replace(/\u200B/g, ""), [])
  ...
  const parseEditor = useCallback((): EditorPart[] => {
    ...
    let buffer = ""
    const flush = () => { if (buffer) { parts.push({ type: "text", content: buffer }); buffer = "" } }
    const walk = (node: Node) => {
      if (node.nodeType === Node.TEXT_NODE) { buffer += node.textContent ?? ""; return }
      if (node.nodeType !== Node.ELEMENT_NODE) return
      const el = node as HTMLElement
      if (el.dataset.type === "file") { flush(); parts.push({ type: "file", content: el.textContent ?? "", path: el.dataset.path }); return }
      if (el.tagName === "BR") { buffer += "\n"; return }
      for (const child of Array.from(el.childNodes)) walk(child)
    }
    Array.from(el.childNodes).forEach((child, i, arr) => {
      const isBlock = child.nodeType === Node.ELEMENT_NODE && ["DIV", "P"].includes((child as HTMLElement).tagName)
      walk(child); if (isBlock && i < arr.length - 1) buffer += "\n"
    })
    flush()
  ...
  const scrollCursorIntoView = useCallback(() => {
    const container = scrollRef.current; const el = editorRef.current
    ...
    if (cursor >= editorText().length) { container.scrollTop = container.scrollHeight; return }
    ...
    const pad = 12
    if (top < container.scrollTop + pad) { container.scrollTop = Math.max(0, top - pad); return }
    if (bottom > container.scrollTop + container.clientHeight - 56) container.scrollTop = bottom - container.clientHeight + 56
  }, [getCursor, editorText])

  const queueScroll = useCallback((count = 2) => {
    requestAnimationFrame(() => { scrollCursorIntoView(); if (count > 1) queueScroll(count - 1) })
  ...
  const renderEditor = useCallback((parts: EditorPart[]) => {
    ...
    for (const part of parts) {
      if (part.type === "text") { el.appendChild(createTextFragment(part.content)); continue }
      if (part.type === "file") {
        ...
        pill.style.userSelect = "text"; pill.style.cursor = "default"
        el.appendChild(pill)
      }
    }
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    for (const file of Array.from(files)) { const att = await fileToAttachment(file); if (att) addAttachment(att) }
  }, [addAttachment])

  // ─── @mentions ──────────────────────────────────────────

  const closeMention = useCallback(() => setMentionState((m) => m.active ? { ...m, active: false, matches: [] } : m), [])
  const setMentionSelected = useCallback((index: number) => setMentionState((s) => ({ ...s, selected: index })), [])

  const onAtInput = useCallback((query: string) => {
    setMentionState((prev) => ({ ...prev, active: true, selected: 0, loading: true }))
    window.vibe.fs.find(workspace, query, 30).then((res) => {
      setMentionState((prev) => !prev.active ? prev : !res.ok ? { ...prev, matches: [], loading: false } : { ...prev, matches: res.matches, loading: false })
    })
  }, [workspace])
  ...
  const handleEditorInput = useCallback(() => {
    ...
    if (!text && attachments.length === 0) {
      setPopover(null); setHistoryIndex(-1); setSavedText(null)
    ...
    if (mode === "normal") {
      ...
      if (atMatch) { onAtInput(atMatch[1]!); setPopover("at") }
      else if (slashMatch) { setPopover("slash") }
      else { closeMention(); setPopover(null) }
    } else setPopover(null)
    setHistoryIndex(-1)
    queueScroll()
  }, [mode, attachments.length, dirty, editorText, getCursor, onAtInput, queueScroll])

  const addPartAtCursor = useCallback((part: EditorPart) => {
    const el = editorRef.current; if (!el) return
    const sel = window.getSelection(); if (!sel) return
    if (sel.rangeCount === 0 || !el.contains(sel.anchorNode)) {
      el.focus(); const end = document.createRange()
    ...
    if (!el.contains(range.startContainer)) return
    if (part.type === "file") {
      const cursor = getCursor(); const text = editorText()
      const beforeCursor = text.slice(0, cursor)
      const atMatch = beforeCursor.match(/@(\S*)$/)
      const pill = createPill(part.path ?? part.content, part.content); const gap = document.createTextNode(" ")
      if (atMatch) { const start = atMatch.index ?? cursor - atMatch[0].length; setRangeEdge(el, range, "start", start); setRangeEdge(el, range, "end", cursor) }
      range.deleteContents(); range.insertNode(gap); range.insertNode(pill)
      range.setStartAfter(gap); range.collapse(true)
      sel.removeAllRanges(); sel.addRange(range)
    }
    if (part.type === "text") {
      const fragment = createTextFragment(part.content); const last = fragment.lastChild
      ...
      if (last) {
        ...
        if (last.nodeType !== Node.TEXT_NODE) {
          ...
          const next = last.nextSibling; const emptyText = next?.nodeType === Node.TEXT_NODE && (next.textContent ?? "") === ""
          if (isBreak && (!next || emptyText)) {
            const ph = next && emptyText ? next : document.createTextNode("\u200B")
            if (!next) last.parentNode?.insertBefore(ph, null)
            ph.textContent = "\u200B"; range.setStart(ph, 0)
  ...
  const applyMention = useCallback((match: FileMatch) => {
    closeMention()
    const el = editorRef.current; if (!el) return
    const text = editorText(); const cursor = getCursor()
    ...
    if (sel && sel.rangeCount > 0) {
      const range = sel.getRangeAt(0)
      setRangeEdge(el, range, "start", atStart); setRangeEdge(el, range, "end", cursor)
    ...
    addAttachment({ id: newAttachmentId(), kind: IMAGE_RE.test(match.path) ? "image" : "file", path: match.path, name: match.name })
  ...
  const submit = useCallback(() => {
    ...
    if (text.startsWith("/")) {
      clearEditor(); setAttachments([]); setDirty(false); setHistoryIndex(-1)
      onSubmit({ slash: text }); return
    }
    const parts: import("../../types.js").ContentPart[] = []
    if (text) parts.push({ type: "text", text })
    for (const a of attachments) { if (a.kind === "image" && a.dataUrl) parts.push({ type: "image_url", image_url: { url: a.dataUrl } }) }
    ...
    if (fileRefs.length > 0) {
      ...
      if (idx >= 0) parts[idx] = { type: "text", text: merged }; else parts.unshift({ type: "text", text: merged })
    }
    setHistoryEntries((prev) => prependHistoryEntry(prev, text, attachments))
    clearEditor(); setAttachments([]); setDirty(false); setHistoryIndex(-1); setSavedText(null); setMode("normal"); setPopover(null)
  ...
  const navigateHistory = useCallback((direction: "up" | "down"): boolean => {
    const text = editorText()
    const result = navigatePromptHistory({ direction, entries: historyEntries, historyIndex, currentText: text, savedText })
    if (!result.handled) return false
    setHistoryIndex(result.historyIndex); setSavedText(result.savedText)
    const el = editorRef.current; if (!el) return true
    if (result.entry.text) setEditorText(result.entry.text); else clearEditor()
  ...
  const selectPopoverActive = useCallback(() => {
    if (popover === "at") { const m = mentionState.matches[mentionState.selected]; if (m) applyMention(m); return }
    if (popover === "slash") { const cmd = slashMatches[slashSelected]; if (cmd) { setEditorText(cmd.name + " "); focusEditorEnd() } }
  }, [popover, mentionState, slashMatches, slashSelected, applyMention, setEditorText, focusEditorEnd])

  // ─── key handling ───────────────────────────────────────

  const isImeComposing = useCallback((event: KeyboardEvent) => event.isComposing || composing || event.keyCode === 229, [composing])

  const handleKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    if ((event.metaKey || event.ctrlKey) && !event.altKey && !event.shiftKey && event.key.toLowerCase() === "u") {
      event.preventDefault(); if (mode !== "normal") return; fileInputRef.current?.click(); return
    }
    if (event.key === "Escape") {
      if (popover) { setPopover(null); event.preventDefault(); event.stopPropagation(); return }
      if (mode === "shell") { setMode("normal"); event.preventDefault(); event.stopPropagation(); return }
      return
    }
    if (event.key === "!" && mode === "normal" && getCursor() === 0) { setMode("shell"); setPopover(null); event.preventDefault(); return }
    if (mode === "shell" && event.key === "Backspace" && getCursor() === 0 && editorText().length === 0) { setMode("normal"); event.preventDefault(); return }
    if (event.key === "Enter" && event.shiftKey) { addPartAtCursor({ type: "text", content: "\n" }); event.preventDefault(); return }
    ...
    if (popover) {
      ...
      if (nav || ctrlNav) {
        ...
        if (popover === "at") {
          ...
          if (event.key === "ArrowDown" || (ctrl && event.key === "n")) setMentionState((s) => ({ ...s, selected: (s.selected + 1) % len }))
          else if (event.key === "ArrowUp" || (ctrl && event.key === "p")) setMentionState((s) => ({ ...s, selected: (s.selected - 1 + len) % len }))
          else if (event.key === "Enter") { const m = mentionState.matches[mentionState.selected]; if (m) applyMention(m) }
        }
        if (popover === "slash") {
          const len = Math.max(slashMatches.length, 1)
          if (event.key === "ArrowDown" || (ctrl && event.key === "n")) setSlashSelected((s) => (s + 1) % len)
          else if (event.key === "ArrowUp" || (ctrl && event.key === "p")) setSlashSelected((s) => (s - 1 + len) % len)
          else if (event.key === "Enter") { const cmd = slashMatches[slashSelected]; if (cmd) { setEditorText(cmd.name + " "); setSlashSelected(0); focusEditorEnd() } }
        }
        return
      }
    }
    if (event.key === "ArrowUp" || event.key === "ArrowDown") {
      ...
      const cursor = getCursor(); const text = editorText(); const dir = event.key === "ArrowUp" ? "up" : "down"
      if (!canNavigateHistoryAtCursor(dir, text, cursor, historyIndex >= 0)) return
      if (navigateHistory(dir)) event.preventDefault(); return
    }
    if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); if (event.repeat) return; submit() }
  }, [mode, popover, composing, getCursor, editorText, addPartAtCursor, navigateHistory, submit, mentionState, slashMatches, slashSelected, historyIndex, isImeComposing, selectPopoverActive, setEditorText, focusEditorEnd])
  ...
  const handlePaste = useCallback(async (event: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    const files = Array.from(cd.items).flatMap((item) => { if (item.kind !== "file") return []; const f = item.getAsFile(); return f ? [f] : [] })
    if (files.length > 0) { await handleFiles(files); return }
    const plainText = cd.getData("text/plain") ?? ""
    if (!plainText) return
    const text = normalizePaste(plainText)
    const put = () => addPartAtCursor({ type: "text", content: text })
    if (pasteMode(text) === "manual") { put(); return }
    const inserted = typeof document.execCommand === "function" && document.execCommand("insertText", false, text)
    if (inserted) return; put()
  }, [handleFiles, addPartAtCursor])

  // ─── composition ────────────────────────────────────────

  const handleCompositionStart = useCallback(() => setComposing(true), [])
  const handleCompositionEnd = useCallback(() => { setComposing(false); requestAnimationFrame(() => { if (composing) return; handleEditorInput() }) }, [handleEditorInput])
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    const vibePath = dt.getData("application/x-vibe-path"); const vibeName = dt.getData("application/x-vibe-name")
    if (vibePath) {
      let rel = vibePath; if (vibePath.startsWith(workspace)) rel = vibePath.slice(workspace.length).replace(/^[\\/]/, "")
      addPartAtCursor({ type: "file", content: "@" + rel, path: vibePath })
      addAttachment({ id: newAttachmentId(), kind: "file", path: vibePath, name: vibeName || rel.split(/[\\/]/).pop() || rel }); return
    }
  ...
  useEffect(() => { if (!disabled) editorRef.current?.focus() }, [disabled])

  // ─── derived state ──────────────────────────────────────

  const imageAttachments = useMemo(() => attachments.filter((a): a is Attachment & { kind: "image" } => a.kind === "image"), [attachments])
  ...
  const placeholderVisible = !disabled && !dirty && !editorText().length
  ...
  return (
    <div className="prompt-input-container">
      ...
      <SlashPopup visible={popover === "slash"} matches={slashMatches} selectedIndex={slashSelected} onSelect={(cmd) => { setEditorText(cmd.name + " "); setSlashSelected(0); editorRef.current?.focus() }} onHover={setSlashSelected} />
      ...
      >
        ...
        {contextItems.length > 0 && (
          <PromptContextItems items={contextItems} remove={(key: string) => setAttachments((prev) => prev.filter((a) => a.id !== key))} t={(s: string) => s} />
        )}
        ...
        >
          ...
          <div className="prompt-input__scroll" ref={scrollRef} style={{ scrollPaddingBottom: space }}>
            ...
            <div
              ...
              onBlur={() => { setFocused(false); setPopover(null); setComposing(false) }}
          ...
          {/* Attach button (bottom-left, matching OpenCode v1 layout) */}
      ...
      {/* ── DockTray (agent/model/variant controls + shell indicator) ── */}
      <div data-dock-surface="tray" data-dock-attach="top" className="prompt-input__tray">
        <div className="prompt-input__tray-inner">
          {/* Shell mode indicator (animated) */}
          <div className="prompt-input__tray-shell" style={shell}>
            ...
            <button
              type="button"
              className="prompt-input__tray-cancel"
          ...
          <div className="prompt-input__tray-controls" style={control}>
            <ModelSelector
