...
export function FileBadge({ info }: { info: FileBadgeInfo }): React.ReactElement {
  return (
    <span className="fbadge">
      {info.cls === "dir" ? (
        <FolderIcon open={false} name={info.name} />
      ) : (
...
function FileChangeRow({ change, cwd }: { change: FileChangeInfo; cwd?: string }) {
  ...
  return (
    <div className="changes-pill">
      <div className={`changes-pill__bar${open ? " changes-pill__bar--open" : ""}`} role="button" tabIndex={0} onClick={() => setOpen(!open)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setOpen(!open); } }}>
...
export function ThinkingBlock({ reasoning, reasoningDone }: {
  reasoning: string;
  ...
  return (
    <div className="thinking-block">
      <div className="thinking-content">
        <Markdown content={displayReasoning} isAssistant={true} simplifiedCodeBlocks={!reasoningDone} noFileIcons={true} />
      </div>
    </div>
  );
}
...
export function VibingLoader({ text }: { text?: string }): React.ReactElement {
  const { t } = useI18n();
  const phrases = [t("vibing"), t("catchingFlow"), t("brewingCode"), t("feelingLogic"), t("chillingBits"), t("surfingSyntax")];
