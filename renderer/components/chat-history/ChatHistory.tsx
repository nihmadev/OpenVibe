...
import { MessageFooter, VibingLoader, ThinkingBlock, UserMessageActions, ToolGroup } from "./ChatHistorySubComponents.js";
...
const MessageItem = React.memo(({ 
  ...
  const { t } = useI18n();
  if (item.kind === "tool") return <AgentToolView item={item} />;
  ...
  const showFooter = item.kind === "assistant" && !isStreaming && item.text.length > 0 && isLastAssistantInTurn && isTurnFinished;
  ...
  return (
    <div className={cls}>
      ...
      <Markdown content={item.text} isAssistant={item.kind === "assistant"} />
      {isStreaming && item.text.length === 0 && !item.reasoning && (
        <VibingLoader />
      )}
      {showFooter && (
        <MessageFooter item={item} items={items} onRegenerate={onRegenerate} cwd={cwd} />
      )}
    </div>
  );
}, (prev, next) => {
...
export function ChatHistory({ items, onPickModel, onRegenerate, onRevert, streamingId, busy, cwd }: Props): React.ReactElement {
