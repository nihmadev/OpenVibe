...
let currentConfig: {
  model: string; baseUrl: string; cwd: string; autoApprove: boolean;
  apiKey: string; apiUrl?: string; providerId?: string;
} | null = null;

// Callback arrays replacing SessionBus
let eventListeners: Array<(e: VibeEvent) => void> = [];
let busyListeners: Array<(b: boolean) => void> = [];
let confirmListeners: Array<(r: ConfirmPayload) => void> = [];
...
async function initVibeBridge() {
  ...
  listen("vibe:agent:assistant-start", () => {
    emitEvent({ kind: "assistant-start" });
  });
  listen<any>("vibe:agent:assistant-chunk", (e) => {
    emitEvent({ kind: "assistant-chunk", text: e.payload.text ?? "" });
  });
  listen<any>("vibe:agent:reasoning-chunk", (e) => {
    emitEvent({ kind: "reasoning-chunk", text: e.payload.text ?? "" });
  });
  listen("vibe:agent:reasoning-end", () => {
    emitEvent({ kind: "reasoning-end" });
  });
  listen("vibe:agent:assistant-end", () => {
    emitEvent({ kind: "assistant-end" });
  });
  listen<any>("vibe:agent:tool-call", (e) => {
    emitEvent({
      kind: "tool-call",
      id: e.payload.id ?? "",
      name: e.payload.name ?? "",
  ...
  listen<any>("vibe:agent:tool-result", (e) => {
    emitEvent({
      kind: "tool-result",
      id: e.payload.id ?? "",
  ...
  listen<any>("vibe:agent:tool-denied", (e) => {
    emitEvent({
      kind: "tool-denied",
      id: e.payload.id ?? "",
      name: e.payload.name ?? "",
  ...
  listen<any>("vibe:agent:confirm-request", (e) => {
    emitConfirm({
      id: e.payload.id ?? "",
  ...
  listen<any>("vibe:agent:busy", (e) => {
    emitBusy(e.payload.busy ?? false);
  });
  listen("vibe:agent:done", async () => {
    emitEvent({ kind: "done" });
...
export const vibe = {
    ...
    new: async () => {
      if (activeChatId) {
        try {
          const msgs = await invoke("agent_get_messages");
          await invoke("chats_save", { id: activeChatId, messages: msgs });
          await invoke("chats_clear", { id: activeChatId });
        } catch {}
      }
      ...
      if (activeChatId) {
        // Return existing chat summary (same id, now empty)
        const list = await invoke<any[]>("chats_list");
        const current = list.find((c: any) => c.id === activeChatId);
        if (current) return current;
      }

      // Fallback: no active chat or it doesn't exist anymore – create a new one
      const result = await invoke<any>("chats_new");
      if (result) {
        activeChatId = result.id;
        localStorage.setItem("openvibe:activeChatId", activeChatId);
      }
      return result;
    },
  ...
  fs: {
    list: (dir: string) =>
      wrap(() => invoke("fs_list", { dir }), (entries) => ({ entries })),
    reveal: async (path: string) => {
      try { await shellOpen(path); } catch {}
    },
    read: (path: string) =>
      wrap(() => invoke<string>("fs_read", { path }), (content) => ({ content })),
    readBinary: (path: string) =>
      wrap(() => invoke<{ data: string; size: number }>("fs_read_binary", { path })),
    write: (path: string, content: string) =>
      wrap(() => invoke("fs_write", { path, content })),
    rename: (from: string, to: string) =>
      wrap(() => invoke("fs_rename", { from, to })),
    delete: (filePath: string) =>
      wrap(() => invoke("fs_delete", { path: filePath })),
    createFile: (dir: string, name: string) =>
      wrap(() => invoke<string>("fs_create_file", { dir, name }), (path) => ({ path })),
    createDir: (dir: string, name: string) =>
      wrap(() => invoke<string>("fs_create_dir", { dir, name }), (path) => ({ path })),
    find: (root: string, query: string, limit?: number) =>
      wrap(() => invoke("fs_find", { root, query, limit }), (matches) => ({ matches })),
    findAll: (root: string, query: string, limit?: number) =>
      wrap(() => invoke("fs_find_all", { root, query, limit }), (matches) => ({ matches })),
  ...
  term: {
    start: (id: string, cols: number, rows: number) =>
      invoke("term_start", { id, cols, rows }),
