...
impl ChatStore {
    ...
    fn migrate(&self) {
        ...
        for (chat_id, json) in &rows {
            ...
            self.conn
                .execute("UPDATE chats SET messages = '[]' WHERE id = ?", params![chat_id])
                .ok();
        }
    }
    ...
    pub fn get(&self, id: &str) -> Option<ChatRecord> {
        ...
        let mut msg_stmt = self
            ...
            .unwrap();
        let msgs: Vec<llm::ChatMessage> = msg_stmt
        ...
        for msg in &record.messages {
            self.conn.execute(
                ...
                params![
                    ...
                    msg.name,
                    msg.tool_call_id,
                    msg.tool_calls.as_ref().map(|tc| serde_json::to_string(tc).unwrap_or_default()),
