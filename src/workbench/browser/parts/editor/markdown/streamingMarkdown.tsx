...
import { CopyCheckIcon } from "@/base/browser/ui/icons/iconRegistry";
import { getFileIcon } from "@/base/browser/ui/icons/iconResolver";
import { Tooltip } from "@/base/browser/ui/tooltip/tooltip";
import { escapeHtml } from "@/base/common/string";
...
import { CodeBlock } from "@/workbench/browser/parts/editor/codeBlock/codeBlock";
...
export const StreamingMarkdown = React.memo(function StreamingMarkdown({
  ...
  React.useEffect(() => {
    appState.get("settings:renderFileTree").then((val) => {
...
function renderTable(rows: string[], noFileIcons: boolean, pk: string, startIndex: number): React.ReactElement {
  ...
  return (
    <div
      ...
      key={`${pk}t${startIndex}`}
      role="region"
      // biome-ignore lint/a11y/noNoninteractiveTabindex: Wide tables require keyboard-accessible horizontal scrolling.
      tabIndex={0}
    >
      <table>
        <thead>
