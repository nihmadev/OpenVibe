...
import { ImageViewer, isImageFile } from "@/workbench/browser/parts/editor/viewers/imageViewer";
import { isVideoFile, VideoViewer } from "@/workbench/browser/parts/editor/viewers/videoViewer";
...
export function EditorArea({
  ...
  const breadcrumb = React.useMemo(() => {
    ...
    if (fileNorm.startsWith("git-diff:") || fileNorm.startsWith("agent-diff:")) {
      try {
    ...
    <div className="editor-area">
      ...
      <div ref={tabsRef} className="editor-area__tabs" role="tablist" onWheel={onTabsWheel}>
        {openFiles.map((path) => {
          ...
          const isDiff = path.startsWith("git-diff:") || path.startsWith("agent-diff:");
      ...
      <div className="editor-area__body">
        {activeFile ? (
          activeFile.startsWith("agent-diff:") ? (
            <AgentDiffViewer key={activeFile} path={activeFile} />
