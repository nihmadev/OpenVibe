...
import { useShortcuts } from "@/platform/keybinding/browser/useKeybindings";
import type { useWorkspaceLayout } from "@/workbench/browser/useWorkbenchLayout";
...
export function useAppCommands(layout: WorkspaceLayout, actions: AppCommandActions) {
  ...
  const { shortcuts, updateBinding, resetBinding } = useShortcuts({
    ...
    focusInput: () => {
      const el = document.querySelector<HTMLElement>('[data-component="composer-input"]');
      el?.focus();
    },
