...
import { computeFlatRows, computeTreeNodes, type FlatRow, type TreeNode } from "../common/search";
import { searchService } from "../tauri/searchService";
import { useSearchKeyboardNav } from "./useSearchKeyboardNav";
import { useSearchShortcuts } from "./useSearchShortcuts";

export type { FlatRow } from "../common/search";
...
export function useCodeSearch({ cwd, onOpenFile, onClose }: UseCodeSearchProps) {
  ...
  const loadFileMatches = useCallback(async (path: string) => {
    ...
    try {
      const res = await searchService.fileMatches(
        p.cwd,
        p.query,
        {
  ...
  const scheduleSearch = useCallback(
    (q: string) => {
      ...
      debounceRef.current = setTimeout(async () => {
        ...
        setFileMatchesMap({});
        try {
          const res = await searchService.searchFiles(
            p.cwd,
            queryStr.trim(),
            {
              matchCase: p.matchCase,
