/**
 * Markdown rendering utility for Composer.
...
function createSyntaxSpan(text: string): HTMLSpanElement {
  ...
  span.className = "composer__md-syntax";
...
function parseInlineMarkdown(text: string, target: Node, options?: MarkdownOptions) {
  ...
  while (match !== null) {
    ...
    if (code) {
      ...
      codeEl.className = "composer__md-code";
      ...
      const span = document.createElement("span");
      span.className = "composer__md-bold";
      ...
      const span = document.createElement("span");
      span.className = "composer__md-italic";
      if (showSyntax) span.appendChild(createSyntaxSpan(sym));
      parseInlineMarkdown(content, span, options);
      if (showSyntax) span.appendChild(createSyntaxSpan(sym));
      target.appendChild(span);
      ...
      const span = document.createElement("span");
      span.className = "composer__md-strike";
      if (showSyntax) span.appendChild(createSyntaxSpan(sym));
      parseInlineMarkdown(content, span, options);
      if (showSyntax) span.appendChild(createSyntaxSpan(sym));
      target.appendChild(span);
      ...
      if (linkMatch) {
        ...
        const span = document.createElement("span");
        span.className = "composer__md-link";
