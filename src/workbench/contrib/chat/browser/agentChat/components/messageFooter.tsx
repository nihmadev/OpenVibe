...
import { resolveMonacoLang } from "@/workbench/browser/parts/editor/codeBlock/codeBlock";
  ...
  return (
    <div className="changes-summary">
      <div className="changes-summary__header">
        <span className="changes-summary__title">{t("filesChanged", { count: changes.length })}</span>
        {/* ZCode pattern: collapsed glance list — file chips joined by styled
            commas, each opening its diff on click. */}
        <span className="tool__chips">
          {changes.map((change, index) => {
            const relPath = toRelativePath(change.filePath, cwd);
            const lastItem = change.items[change.items.length - 1];
            const clickable = Boolean(onOpenAgentDiff && lastItem);
            const open = () => {
              if (onOpenAgentDiff && lastItem) onOpenAgentDiff(lastItem.id, change.filePath);
            };
            return (
              <span className="tool__chip" key={change.filePath}>
                {index > 0 && <span className="tool__chip-sep">,</span>}
                {clickable ? (
                  <button type="button" className="tool__chip-btn" title={relPath} onClick={open}>
                    <FileIcon name={basename(relPath)} />
                    <span className="tool__chip-name">{basename(relPath)}</span>
                  </button>
                ) : (
                  <span className="tool__chip-btn" title={relPath}>
                    <FileIcon name={basename(relPath)} />
                    <span className="tool__chip-name">{basename(relPath)}</span>
                  </span>
                )}
