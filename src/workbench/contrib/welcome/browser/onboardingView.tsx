...
import { type ThemeDef, themes } from "@/platform/theme/themeRegistry";
import { useTheme } from "@/platform/theme/themeService";
import { InlineAnimPreview } from "@/workbench/browser/animationPreview";
import "./onboardingView.css";
import { getSystemUser } from "@/platform/native/tauri/systemUserService";
...
function OnboardingViewContent({ onComplete, onLanguageChange }: OnboardingViewProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const loadSetting = async (key: string, setter: (v: any) => void, _def: string) => {
      try {
        const v = await appState.get(key);
  ...
  const handleProxyToggle = (checked: boolean) => {
    setUseProxy(checked);
    try {
      appState.set("settings:useRegionalProxy", String(checked));
    } catch (error) {
      void error;
    }
  };
  ...
  const handleAutoAcceptToggle = (checked: boolean) => {
    ...
    try {
      appState.set("settings:autoAccept", String(checked));
    } catch (error) {
      void error;
    }
  };
  ...
  const updateBooleanSetting = (
    ...
    setter(checked);
    try {
      appState.set(`settings:${key}`, String(checked));
    } catch (error) {
      void error;
    }
  };
  ...
  const handleShellChange = (val: string) => {
    ...
    try {
      appState.set("settings:terminalShell", val);
    } catch (error) {
      void error;
    }
  };
  ...
  const handleFontChange = (val: string) => {
    ...
    try {
      appState.set("settings:font", val);
    } catch (error) {
      void error;
    }
  };
  ...
  const handleEditorFontSizeChange = (val: string) => {
    ...
    try {
      appState.set("settings:editorFontSize", val);
    } catch (error) {
      void error;
    }
  };
                  ...
                  <div className="ws-setting-control">
                    <Select
                      ...
                      onChange={(newLang) => {
                        ...
                        try {
                          appState.set("settings:language", newLang);
                        } catch (error) {
                          void error;
                        }
                      }}
