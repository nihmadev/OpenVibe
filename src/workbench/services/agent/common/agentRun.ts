import type { HistoryItem, TodoCheckpoint, TodoTask } from "@/workbench/common/conversation";
import { basename, getEditStrings, getFilePathFromArgs } from "./agentToolPresentation";
...
/** Aggregate successful edits in the current turn for the composer status pill. */
export function currentRunFileChangeSummary(items: HistoryItem[]): RunFileChangeSummary | null {
  ...
  for (let index = items.length - 1; index >= 0; index--) {
    ...
    if (item.kind !== "tool" || item.ok !== true) continue;
    ...
    if (!path) continue;
    const strings = getEditStrings(item.toolArgs);
    paths.add(path);
    added += strings.newStr.split("\n").filter(Boolean).length;
    if (!isWrite) removed += strings.oldStr.split("\n").filter(Boolean).length;
  }

  return paths.size > 0 ? { fileCount: paths.size, added, removed } : null;
}
...
export function toolActivityTitle(item: HistoryItem, t: Translate): string {
  ...
  if (item.toolName?.startsWith("browser_")) {
    const target = stringArg(item, "url") || stringArg(item, "ref");
    if (item.toolName === "browser_open") return t("browserActivityOpen", { target: target || "browser" });
    if (item.toolName === "browser_navigate") return t("browserActivityNavigate", { target: target || "page" });
    if (item.toolName === "browser_click") return t("browserActivityClick", { target: target || "element" });
    if (item.toolName === "browser_fill" || item.toolName === "browser_type") {
      return t("browserActivityFill", { target: target || "field" });
    }
    return t("browserActivityVerify");
  }

  if (item.toolName === "list_skills") {
    return item.ok === undefined ? t("listSkillsActive") : t("listSkillsDone");
  }
  if (item.toolName === "read_skill") {
    const name = stringArg(item, "name") || "skill";
    return item.ok === undefined ? t("readSkillActive", { name }) : t("readSkillDone", { name });
  }
  if (item.toolName === "read_skill_resource") {
    const skill = stringArg(item, "name") || "skill";
    const resource = stringArg(item, "resource");
    const name = resource ? `${skill}/${resource}` : skill;
    return item.ok === undefined ? t("readSkillResourceActive", { name }) : t("readSkillResourceDone", { name });
  }
...
export function toolActivityKind(item: HistoryItem): ToolActivityKind {
  ...
  if (item.toolName?.startsWith("browser_")) return "browser";
  if (item.toolName === "list_skills" || item.toolName === "read_skill" || item.toolName === "read_skill_resource") {
    return "skill";
  }
  switch (item.toolName) {
    case "search_codebase":
...
export type RunTimelineNode =
  ...
  | { type: "browser"; id: string; items: HistoryItem[] }
...
export function buildRunTimeline(items: HistoryItem[], finalItemId?: string): RunTimelineNode[] {
  ...
  // Second pass: browser calls form one agent-activity-group; research steps
  // retain their existing Analysis grouping.
  const grouped: RunTimelineNode[] = [];
  let burst: RunFlowTool[] = [];
  let browserBurst: HistoryItem[] = [];
  ...
  const flushBrowserBurst = () => {
    if (browserBurst.length === 0) return;
    grouped.push({ type: "browser", id: browserBurst[0]!.id, items: browserBurst });
    browserBurst = [];
  };
  ...
  for (const node of nodes) {
    ...
    if (node.type === "tool" && node.kind === "browser") {
      flushBurst();
      browserBurst.push(...node.items);
      continue;
    }
    flushBurst();
    flushBrowserBurst();
...
const ACTIVITY_SUMMARY_LABELS: Partial<Record<ToolActivityKind, string>> = {
  ...
  browser: "activitySummaryBrowser",
  skill: "activitySummarySkills",
...
export function activitySummaryLabel(nodes: RunTimelineNode[], t: Translate): string | null {
  ...
  for (const node of nodes) {
    ...
    if (node.type === "browser") addKind("browser");
  }

  // Codex summaries use a stable semantic order instead of event order.
  // This is what keeps the completed label predictable: exploration appears
  // before commands, producing e.g. “Read files and ran commands”.
