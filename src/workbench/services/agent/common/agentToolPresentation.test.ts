...
suite("describe", () => {
  it("describes the skill connection and loaded instruction phases", () => {
    const pending = {
      toolName: "read_skill",
      toolArgs: { name: "browser-control" },
    } as HistoryItem;
    const done = { ...pending, ok: true } as HistoryItem;

    expect(describeItem(pending).verb).toBe("Connecting to skill browser-control");
    expect(describeItem(done).verb).toBe("Loaded instructions for skill browser-control");
  });

  it("describes list_skills and skill resources without raw tool names", () => {
    expect(describeItem({ toolName: "list_skills", ok: true } as HistoryItem).verb).toBe("Loaded the available skills");
    expect(
      describeItem({
        toolName: "read_skill_resource",
        toolArgs: { name: "browser-control", resource: "references/auth.md" },
        ok: true,
      } as HistoryItem).verb,
    ).toBe("Read a resource for skill browser-control/references/auth.md");
  });
