...
export function describe(
  ...
  const label = (key: string, fallback: string, params?: Record<string, string | number | boolean>): string => {
  ...
  switch (item.toolName) {
    ...
    case "list_skills":
      return {
        verb: pending
          ? label("listSkillsActive", "Checking available skills")
          : label("listSkillsDone", "Loaded the available skills"),
        file: null,
        suffix: "",
      };
    case "read_skill": {
      const args = item.toolArgs as { name?: string } | undefined;
      const name = args?.name?.trim() || "skill";
      return {
        verb: pending
          ? label("readSkillActive", "Connecting to skill {name}", { name })
          : label("readSkillDone", "Loaded instructions for skill {name}", { name }),
        file: null,
        suffix: "",
      };
    }
    case "read_skill_resource": {
      const args = item.toolArgs as { name?: string; resource?: string } | undefined;
      const skill = args?.name?.trim() || "skill";
      const resource = args?.resource?.trim();
      const name = resource ? `${skill}/${resource}` : skill;
      return {
        verb: pending
          ? label("readSkillResourceActive", "Reading a resource for skill {name}", { name })
          : label("readSkillResourceDone", "Read a resource for skill {name}", { name }),
        file: null,
        suffix: "",
      };
    }
