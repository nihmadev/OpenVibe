...
const t = (key: string, params?: Record<string, string | number | boolean>) => {
  const labels: Record<string, string> = {
    ...
    readSkillActive: `Connecting to skill ${params?.name}`,
    readSkillDone: `Loaded instructions for skill ${params?.name}`,
    tool: "Tool",
  };
...
describe("agent run presentation", () => {
  ...
  it("renders skill loading as a distinct localized activity", () => {
    const skill = item({
      id: "skill",
      kind: "tool",
      toolName: "read_skill",
    ...
    expect(toolActivityTitle(skill, t)).toBe("Loaded instructions for skill browser-control");
    expect(buildRunTimeline([skill])).toMatchObject([{ type: "tool", kind: "skill" }]);
  });
