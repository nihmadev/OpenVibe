...
import type { BranchInfo, CommitFile, CommitGraphNode, CommitInfo, FileStatus, RepositoryInfo } from "../common/scm";

const data = <T>(r: T) => ({ data: r });

export const gitScmService = {
  repoInfo: (cwd: string): Promise<Result<{ data: RepositoryInfo }>> =>
    wrap(() => invoke<RepositoryInfo>("git_repo_info", { path: cwd }), data),
