...
export function useProjects() {
  ...
  const animateRemove = useCallback(
    async (id: string, onProjectChange?: (folder: string | null, projectId: string | null) => Promise<void>) => {
      ...
      const next = await workspaceService.remove(id);
  ...
  const handlePickProject = useCallback(
    async (id: string, onProjectChange: (folder: string | null, projectId: string | null) => Promise<void>) => {
      ...
      const project = await workspaceService.setActive(id);
      if (!project) return;
  ...
  const handleAddProject = useCallback(
    async (onProjectChange: (folder: string | null, projectId: string | null) => Promise<void>) => {
      const project = await workspaceService.add();
      if (!project) return;
      const list = await workspaceService.list();
      setProjects(list);
      setActiveProject(project.id);
    ...
    async (
      ...
      try {
        ...
        for (const m of missing) {
          const next = await workspaceService.remove(m.id);
