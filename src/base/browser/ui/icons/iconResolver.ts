import { FOLDER_MAP, ICON_MAP, LANGUAGE_MAP } from "./iconMaps";
...
export function getFolderIcon(name: string | undefined, open: boolean): string {
  ...
  // Mentions carry a relative path (for example `src/components`), while the
  // folder map is keyed by the folder's basename.
  const lowerName = name
    .replace(/[\\/]$/, "")
    .split(/[\\/]/)
    .pop()
    ?.toLowerCase();
  if (!lowerName) return open ? "folder-open.svg" : "folder.svg";
