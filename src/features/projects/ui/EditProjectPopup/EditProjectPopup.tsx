...
import { CloseXIcon, UploadIcon } from "@/shared/icons/icons";
...
export function EditProjectPopup({ project, onSave, onClose }: Props): React.ReactElement {
  ...
  const handleSave = async () => {
    ...
    if (!trimmed) return;
    await projectsGateway.rename(project.id, trimmed);
    await projectsGateway.setColor(project.id, color);
    await projectsGateway.setPhoto(project.id, photo || null);
    await projectsGateway.setIcon(project.id, null);
    onSave();
  };
    ...
    <div className="edit-project-overlay" onClick={onClose}>
      <div className="edit-project" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="edit-project__actions">
          <button className="z-button z-button--primary" onClick={handleSave}>
            {t("save")}
          </button>
          <button className="z-button z-button--outline" onClick={onClose}>
            {t("cancel")}
          </button>
        </div>
      </div>
