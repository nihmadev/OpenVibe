...
import { searchGateway } from "../infrastructure/searchGateway";
import { computeFlatRows, computeTreeNodes, type FlatRow, type TreeNode } from "../model/searchTree";
import { useSearchKeyboardNav } from "./useSearchKeyboardNav";
import { useSearchShortcuts } from "./useSearchShortcuts";

export type { FlatRow } from "../model/searchTree";
...
export function useCodeSearch({ cwd, onOpenFile, onClose }: UseCodeSearchProps) {
  ...
  const loadFileMatches = useCallback(async (path: string) => {
    ...
    try {
      const res = await searchGateway.fileMatches(
        p.cwd,
        p.query,
        {
          matchCase: p.matchCase,
          matchWholeWord: p.matchWholeWord,
          useRegex: p.useRegex,
          include: p.includeFilter,
          exclude: p.excludeFilter,
        },
        path,
        0,
        200,
  ...
  const scheduleSearch = useCallback(
    (q: string) => {
      ...
      debounceRef.current = setTimeout(async () => {
        ...
        setFileMatchesMap({});
        try {
          const res = await searchGateway.searchFiles(
            p.cwd,
            queryStr.trim(),
            {
              matchCase: p.matchCase,
              matchWholeWord: p.matchWholeWord,
              useRegex: p.useRegex,
              include: p.includeFilter || undefined,
              exclude: p.excludeFilter || undefined,
            },
            1000,
          );
          if (res.ok && gen === searchGenRef.current) {
            ...
            if (res.files && res.files.length > 0) {
              setCollapsedFiles(new Set(res.files.map((f) => f.path)));
              setCollapsedTree((prev) => {
                const next = new Set(prev);
                for (const file of res.files) next.add(file.rel);
                return next;
              });
            }
          } else {
