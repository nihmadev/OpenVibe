...
export function GitPanel({ cwd, onOpenFile, onClose: _onClose }: GitPanelProps) {
  ...
  const loadCommitFiles = async (hash: string) => {
    ...
    try {
      const res = await gitGateway.commitFiles(cwd, hash);
      if (res.ok && res.data) {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    try {
      const [statusRes, branchRes, curBranchRes, graphRes] = await Promise.all([
        gitGateway.status(cwd),
        gitGateway.branches(cwd),
        gitGateway.currentBranch(cwd),
        gitGateway.graph(cwd, 9999999),
      ]);
  ...
  const handleStageAll = async () => {
    await gitGateway.stageAll(cwd);
    fetchAllData();
  };

  const handleStageFile = async (filePath: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    await gitGateway.stageFile(cwd, filePath);
    fetchAllData();
  };

  const handleUnstageFile = async (filePath: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    await gitGateway.unstageFile(cwd, filePath);
    fetchAllData();
  };

  const handleRevertFile = async (filePath: string, e?: React.MouseEvent) => {
    e?.stopPropagation();
    await gitGateway.revertFile(cwd, filePath);
    fetchAllData();
  };

  const handleCommit = async () => {
    if (!commitMessage.trim()) return;
    const res = await gitGateway.commit(cwd, commitMessage);
    if (res.ok) {
      setCommitMessage("");
      fetchAllData();
    }
  ...
  const handleCheckoutBranch = async (branchName: string) => {
    await gitGateway.checkoutBranch(cwd, branchName);
  ...
  const handleCreateBranch = async () => {
    ...
    await gitGateway.createBranch(cwd, newBranchName.trim());
    await gitGateway.checkoutBranch(cwd, newBranchName.trim());
    setNewBranchName("");
    setShowBranchModal(false);
    fetchAllData();
  };
  ...
  const handleGenerateCommitMessage = async () => {
    ...
    try {
      const res = await gitGateway.generateCommitMessage(cwd);
      if (res.ok && res.data) {
                      ...
                      <div
                        ...
                        onClick={async () => {
                          if (isSelected) {
                            ...
                            const details = await gitGateway.commitDetails(cwd, node.id);
      ...
      {hoveredCommit && (
        <GitCommitTooltip
          ...
          onSelectCommit={async (commit) => {
            const details = await gitGateway.commitDetails(cwd, commit.id);
            if (details.ok && details.data) {
              setSelectedCommit(details.data);
              loadCommitFiles(commit.id);
            }
