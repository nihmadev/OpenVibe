...
import { mentionsFromText, recordToItems } from "./chatHistory";
...
describe("mentionsFromText", () => {
  it("restores file and folder mention metadata from persisted text", () => {
    expect(mentionsFromText("Inspect @src/components and @src/index.ts")).toEqual([
      { display: "src/components", path: "src/components", isDir: undefined },
      { display: "src/index.ts", path: "src/index.ts", isDir: undefined },
    ]);
  });

  it("preserves an explicit trailing separator for folders", () => {
    expect(mentionsFromText("Inspect @src/components/")[0]).toMatchObject({ isDir: true });
  });
});
