...
import { chatsGateway, onChatsUpdated } from "@/features/chats/infrastructure/chatsGateway";
import type { ChatRecord, ChatSummary } from "../model/chat";
...
export function useChats() {
  ...
  const loadChatList = useCallback(async () => {
    const all = await chatsGateway.list();
  ...
  const mergeChatList = useCallback(async () => {
    const incoming = (await chatsGateway.list()).filter((c) => c.messageCount > 0);
    setChatsSynced((prev) => {
  ...
  const handlePickChat = useCallback(
    async (id: string, onChatChange: (record: ChatRecord | null) => void) => {
      ...
      if (prevId) {
        ...
        if (!prevInList) {
          chatsGateway.delete(prevId).catch(() => {});
        }
      }

      const record = await chatsGateway.open(id);
  ...
  const handleNewChat = useCallback(
    async (onChatChange: (record: ChatRecord | null) => void) => {
      ...
      try {
        const fresh = await chatsGateway.new();
        if (!fresh) return;

        // Same chat was cleared in-place — just clear UI
        ...
        const record = await chatsGateway.open(fresh.id);
  ...
  const handleCloseChat = useCallback(
    async (id: string, onChatChange: (record: ChatRecord | null) => void) => {
      ...
      await chatsGateway.delete(id);
      const list = await chatsGateway.list();
      if (list.length === 0) {
        const fresh = await chatsGateway.new();
        if (fresh) {
          const record = await chatsGateway.open(fresh.id);
      ...
      if (activeChatRef.current === id) {
        const next = list[0]!;
        const record = await chatsGateway.open(next.id);
