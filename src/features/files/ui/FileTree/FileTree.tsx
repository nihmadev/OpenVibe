...
import "./FileTree.css";
import { fsApi, onFsChanged } from "@/features/files/infrastructure/fsGateway";
import { useI18n } from "@/shared/i18n/useI18n";
import { ChevronRightIcon, CollapseAllIcon, NewFileIcon, NewFolderIcon, RefreshIcon } from "@/shared/icons";
import { ContextMenu, type MenuItem } from "@/shared/ui/ContextMenu/ContextMenu";
import { Tooltip } from "@/shared/ui/Tooltip/Tooltip";
import type { FsEntry } from "../../model/fs";
import { FileNode } from "./FileNode";
import { RenameInput } from "./RenameInput";
import type { CtxState, NodeState, RootProps } from "./types";
...
export function FileTree({ cwd, onOpenFile, activeFile, revealPath }: RootProps): React.ReactElement {
  ...
  async function refreshDir(dir: string): Promise<void> {
    if (dir === cwd) {
      const res = await fsApi.list(cwd);
  ...
  useEffect(() => {
    ...
    (async () => {
      const results = await Promise.all(dirs.map((d) => fsApi.list(d)));
      setStates((prev) => {
        const map = new Map(prev);
  ...
  async function commitCreate(name: string): Promise<void> {
    ...
    for (const part of directoryParts) {
      const listed = await fsApi.list(targetDir);
      const existing = listed.ok ? listed.entries.find((entry) => entry.isDir && entry.name === part) : undefined;
      if (existing) {
        targetDir = existing.path;
        continue;
      }
      res = await fsApi.createDir(targetDir, part);
      if (!res.ok) break;
      targetDir = res.path;
    }
    if (res.ok && kind === "file") {
      res = await fsApi.createFile(targetDir, parts[parts.length - 1]!);
    }
  ...
  async function commitRename(path: string, newName: string): Promise<void> {
    ...
    const res = await fsApi.rename(path, newPath);
    if (!res.ok) {
    ...
    if (entry) {
      ...
      if (cutPath) {
        items.push({
          label: t("paste"),
          onClick: async () => {
            const res = await fsApi.rename(cutPath, `${p}/${basename(cutPath)}`);
      ...
      items.push({
        ...
        onClick: async () => {
          if (confirm(t("deleteConfirm", { name: entry.name }))) {
            const res = await fsApi.delete(p);
            if (!res.ok) alert(res.error);
