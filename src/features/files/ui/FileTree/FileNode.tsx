...
export function FileNode(props: NodeProps): React.ReactElement {
  ...
  function toggleDir(): void {
    ...
    if (cur?.open) {
      ...
      fsApi.list(entry.path).then((res) => {
        setStates((prev) => {
          const map = new Map(prev);
          if (res.ok) {
  ...
  async function selectCompactFolder(path: string): Promise<void> {
    ...
    const res = known?.children ? { ok: true as const, entries: known.children } : await fsApi.list(path);
    setStates((prev) => {
      const map = new Map(prev);
  ...
  async function onDrop(e: React.DragEvent<HTMLDivElement>): Promise<void> {
    ...
    const res = await fsApi.rename(srcPath, destPath);
    if (res.ok) {
      await refreshAll();
    }
  }
