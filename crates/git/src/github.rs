...
pub fn get_profile() -> Result<GitHubProfile> {
    ...
    let profile: GitHubProfile = serde_json::from_slice(&output.stdout)
        .map_err(|e| GitError::Other(e.to_string()))?;
    Ok(profile)
}
...
pub fn get_branches(repo: &str) -> Result<Vec<GitHubBranch>> {
    ...
    let branches: Vec<GitHubBranch> = serde_json::from_slice(&output.stdout)
        .map_err(|e| GitError::Other(e.to_string()))?;
    Ok(branches)
}
...
pub fn get_github_commits(repo: &str) -> Result<Vec<GitHubCommit>> {
    ...
    let commits: Vec<GitHubCommit> = serde_json::from_slice(&output.stdout)
        .map_err(|e| GitError::Other(e.to_string()))?;
    Ok(commits)
}
...
pub fn get_pull_requests(repo: &str, state: &str) -> Result<Vec<GitHubPullRequest>> {
    ...
    let prs: Vec<GitHubPullRequest> = serde_json::from_slice(&output.stdout)
        .map_err(|e| GitError::Other(e.to_string()))?;
    Ok(prs)
}
...
mod tests {
    ...
    fn test_get_profile() {
        ...
        assert!(profile.is_ok(), "Failed to get profile: {:?}", profile.err());
        let profile = profile.unwrap();
        assert!(!profile.login.is_empty(), "Profile login should not be empty");
    }
    ...
    fn test_get_branches() {
        ...
        assert!(branches.is_ok(), "Failed to get branches: {:?}", branches.err());
        let branches = branches.unwrap();
        assert!(!branches.is_empty(), "Expected at least one branch");
        assert!(branches.iter().any(|b| b.name == "main" || b.name == "master"), "Expected main or master branch");
    }
    ...
    fn test_get_github_commits() {
        ...
        assert!(commits.is_ok(), "Failed to get commits: {:?}", commits.err());
