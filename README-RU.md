...
- **`crates/agent-tool`**: Исполнитель системных инструментов (`read_file`, `write_file`, `edit_file`, `list_dir`, `run`, `search_codebase`, `agent` суб-агент) и динамический мост для инструментов Model Context Protocol (`mcp__<server>__<tool>`). Запрашивает подтверждение пользователя перед выполнением команд терминала.
...
- **Встроенные инструменты**: `read_file`, `write_file`, `edit_file`, `list_dir`, `run`, `search_codebase` и `agent` (суб-агент для сложных исследований кода).
